...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo *pCreateInfo, const VkAllocationCallbacks *pAllocator, VkDevice *pDevice)
{
	...
	while(extensionCreateInfo)
	{
		...
		case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SHADER_DRAW_PARAMETERS_FEATURES:
			{
				...
				(void)shaderDrawParametersFeatures->shaderDrawParameters;
			}
			break;
		case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VERTEX_ATTRIBUTE_DIVISOR_FEATURES_EXT:
			{
				const auto *vertexAttributeDivisorFeatures = reinterpret_cast<const VkPhysicalDeviceVertexAttributeDivisorFeaturesEXT *>(extensionCreateInfo);

				if(!vk::Cast(physicalDevice)->hasExtendedFeatures(vertexAttributeDivisorFeatures))
				{
					return VK_ERROR_FEATURE_NOT_PRESENT;
				}
			}
		...
		case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_UNIFIED_IMAGE_LAYOUTS_FEATURES_KHR:
			{
				const auto *unifiedImageLayoutsFeatures = reinterpret_cast<const VkPhysicalDeviceUnifiedImageLayoutsFeaturesKHR *>(extensionCreateInfo);
				bool hasFeatures = vk::Cast(physicalDevice)->hasExtendedFeatures(unifiedImageLayoutsFeatures);
				if(!hasFeatures)
				{
					return VK_ERROR_FEATURE_NOT_PRESENT;
				}
...
{
	TRACE("(VkCommandBuffer commandBuffer = %p, VkPipelineBindPoint pipelineBindPoint = %d, VkPipelineLayout layout = %p, uint32_t set = %d, uint32_t descriptorWriteCount = %d, const VkWriteDescriptorSet* pDescriptorWrites = %p)",
	      commandBuffer, int(pipelineBindPoint), static_cast<void *>(layout), int(set), int(descriptorWriteCount), pDescriptorWrites);

	vk::Cast(commandBuffer)->pushDescriptorSet(pipelineBindPoint, vk::Cast(layout), set, descriptorWriteCount, pDescriptorWrites);
}
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceProperties2(VkPhysicalDevice physicalDevice, VkPhysicalDeviceProperties2 *pProperties)
{
	...
	while(extensionProperties)
	{
		...
		case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PUSH_DESCRIPTOR_PROPERTIES:
			{
				auto *properties = reinterpret_cast<VkPhysicalDevicePushDescriptorProperties *>(extensionProperties);
				vk::Cast(physicalDevice)->getProperties(properties);
			}
			break;
		...
		case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MULTI_DRAW_PROPERTIES_EXT:
			{
				auto *properties = reinterpret_cast<VkPhysicalDeviceMultiDrawPropertiesEXT *>(extensionProperties);
				vk::Cast(physicalDevice)->getProperties(properties);
			}
			break;
